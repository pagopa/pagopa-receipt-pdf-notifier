Feature: All about payment events consumed by Azure functions receipt-pdf-notifier
  ...
  Scenario: a receipt stored on receipt datastore is stored into receipts datastore is notified to a io user
    Given a random receipt with id "receipt-notifier-test-id-1" stored on receipt datastore with generated pdf
